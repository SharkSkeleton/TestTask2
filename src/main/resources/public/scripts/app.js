...
app.controller('ListCtrl', function ($scope, $http) {
    ...
    $scope.delete = function () {
        var shapesToDelete = $scope.todos.filter(todo => todo.deleted);
        console.log('Shapes to delete', shapesToDelete);

        shapesToDelete.forEach(function(shape) {
            $http.delete('/api/v1/todos/' + shape.id).success(function (data) {
                $location.path('/');
            }).error(function (data, status) {
                console.log('Error ' + data)
            })
